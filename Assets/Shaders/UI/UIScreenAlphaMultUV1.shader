Shader "Ballistic/UI/ScreenAlphaMultUV1" {
	Properties
		{
			[PerRendererData] _MainTex ("Sprite Texture", 2D) = "white" {}
			[Gamma] _Exposure ("Exposure", Range(0, 20)) = 1.0
			_AlphaTex ("Alpha Texture", 2D) = "white" {}
			_AlphaTex2 ("Alpha Texture UV1", 2D) = "white" {}
		...
		fixed4 frag(v2f IN) : SV_Target
		{
			...
			fixed alpha2 = tex2D(_AlphaTex2, IN.texcoord1).a;

			color.a *= UnityGet2DClipping(IN.worldPosition.xy, _ClipRect) * alpha * alpha2;
